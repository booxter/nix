...
let
  ...
  appsCfg = userEnvironment.features.apps;
  emailCfg = appsCfg.email;
...
in
lib.mkIf (appsCfg.enable && emailCfg.enable) {
  programs.thunderbird = lib.mkIf emailCfg.thunderbird.enable {
    enable = true;
