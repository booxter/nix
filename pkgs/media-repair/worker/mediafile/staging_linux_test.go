...
func TestStagedArtifactIsPrivateAndOnMediaFilesystem(t *testing.T) {
	...
	if err := unix.Stat(filepath.Join(rootPath, workspaceDirectoryName), &workspaceStat); err != nil {
		t.Fatal(err)
	}
	if workspaceStat.Uid != uint32(os.Geteuid()) || workspaceStat.Mode&0o7777 != 0o750 {
		t.Fatalf(
			"workspace owner and mode = %d, %o",
			workspaceStat.Uid,
			workspaceStat.Mode&0o7777,
		)
	}
...
func TestDiscardRemovesOnlyItsStagedArtifact(t *testing.T) {
	...
	entries, err := os.ReadDir(filepath.Join(rootPath, workspaceDirectoryName, stagedDirectoryName))
	if err != nil {
		t.Fatal(err)
	}
	if len(entries) != 0 {
...
func TestPublishCompletedAtPlacesBluRayOutputBesideBDMV(t *testing.T) {
	...
	if _, err := rootSet.PublishCompletedAt(
		"downloads", "artifact:bluray", workercontracts.OutputContainerMKV,
		fingerprint, []string{".media-repair", "staged"},
	); err == nil {
...
func TestPublishCompletedRejectsUnsafeDestination(t *testing.T) {
	...
	for _, inputPaths := range [][][]string{
		...
		{{workspaceDirectoryName, "first.mkv"}, {workspaceDirectoryName, "second.mkv"}},
	} {
