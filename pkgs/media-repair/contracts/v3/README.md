...
Radarr diagnostics remain bounded before they cross the planner boundary.
`status_message_count` records how many status entries Radarr returned, while
each entry's `message_count` records its original number of detail strings. A
count larger than the corresponding array length means the controller omitted
excess diagnostics. The planner must treat that evidence as incomplete.
