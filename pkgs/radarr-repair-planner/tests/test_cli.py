...
VALID_CASES = sorted((FIXTURES / "contracts/v1/examples").glob("repair-case-*.json"))
VALID_DECISIONS = sorted((FIXTURES / "contracts/v1/examples").glob("repair-decision-*.json"))
...
def test_valid_decision_examples_round_trip(path: Path) -> None:
    ...
    assert json.loads(encoded)["schema_version"] == "radarr-repair/v1"
