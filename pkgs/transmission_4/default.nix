...
stdenv.mkDerivation (finalAttrs: {
  ...
  patches = [
    ...
    (fetchpatch2 {
      # Backport include fix for newer toolchains:
      # https://github.com/transmission/transmission/commit/ce66e5c4425b6cda9048e7fdb631e097bbdc50e9
      url = "https://github.com/transmission/transmission/commit/ce66e5c4425b6cda9048e7fdb631e097bbdc50e9.patch?full_index=1";
      hash = "sha256-hE3j1x1c+tHXEDVQR22fYTTpVl6I2ExNckLm1KgyzfA=";
      includes = [ "libtransmission/announcer.h" ];
    })
  ];
  ...
  postPatch = ''
    ...
    # fmt >= 12 dropped fmt::ptr(shared_ptr), use raw pointer explicitly.
    substituteInPlace libtransmission/peer-msgs.cc \
      --replace-fail 'fmt::ptr(msgs->io)' 'fmt::ptr(msgs->io.get())'

    # Backport from upstream fmt compatibility work:
    # https://github.com/transmission/transmission/commit/fb25228f24ff
    substituteInPlace libtransmission/session-alt-speeds.cc \
      --replace-fail 'fmt::localtime(time)' '*std::localtime(&time)'

    # Backport from upstream fmt compatibility work:
    # https://github.com/transmission/transmission/commit/fb25228f24ff
    substituteInPlace libtransmission/torrent.cc \
      --replace-fail \
        'auto const bytes_downloaded_str = std::to_string(tor->downloadedCur + tor->downloadedPrev);' \
        'auto const bytes_downloaded_str = std::to_string(tor->downloadedCur + tor->downloadedPrev);
    auto const now = tr_time();' \
      --replace-fail 'fmt::localtime(tr_time())' '*std::localtime(&now)'

    # Backport from upstream fmt compatibility work:
    # https://github.com/transmission/transmission/commit/fb25228f24ff
    substituteInPlace utils/show.cc \
      --replace-fail 'fmt::localtime(now)' '*std::localtime(&now)'
    substituteInPlace utils/remote.cc \
      --replace-fail 'fmt::localtime(now)' '*std::localtime(&now)'
  '';
