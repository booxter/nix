...
func TestExecutorResumesStoredJoin(t *testing.T) {
	...
	tests := []struct {
		...
		{"scan prepared", casestore.JoinScanPrepared, "", casestore.JoinImported, 0, 1},
		{"scan requested", casestore.JoinScanRequested, "", casestore.JoinImported, 0, 1},
...
func joinRecoveryScenario(
	t *testing.T,
) (casebuilder.Assembly, contracts.RepairDecisionV1, decisionpolicy.AuthorizedJoin) {
	t.Helper()
...
func manualRecoveryScenario(
	t *testing.T,
) (casebuilder.Assembly, contracts.RepairDecisionV1, decisionpolicy.AuthorizedManualImport) {
	t.Helper()
	const (
