...
class CommandOciBackend:
    ...
    def verify_signature(self, image: str, digest: str, key: Path) -> None:
        ...
        print(f"verifying signature: cosign verify --key {key} {reference}", file=sys.stderr)
        result = self.runner.run(
            [self.tools.cosign, "verify", "--key", str(key), reference],
            cwd=self.repo_root,
            capture=False,
        )
        if result.returncode != 0:
            raise UpdateError(f"Cosign verification failed for {reference}")
...
def verify_signature(
    ...
    signature_type = pin.signature.type
    if signature_type is None:
        return "not configured"
    if signature_type != "cosign-key":
        raise UpdateError(
            f"Unsupported signature verification type for {pin.image}: {signature_type}"
        )
    if not pin.signature.key:
        raise UpdateError(f"Signature verification for {pin.image} is missing signature.key")
    if "://" in pin.signature.key:
        raise UpdateError(
            f"Signature verification key for {pin.image} must be a vendored local path, "
            f"got: {pin.signature.key}"
        )
    key = Path(pin.signature.key)
    if not key.is_absolute():
        key = repo_root / key
    if not key.is_file():
        raise UpdateError(f"Signature verification key for {pin.image} not found: {key}")
    backend.verify_signature(pin.image, digest, key)
    return "cosign verified"
...
def summary_row(
    ...
    return (
        ...
        f"{markdown_link('compare', comparison)} | `{signature}` |\n"
    )
...
def update_oci_images(
    ...
    with summary_file.open("a") as summary:
        for name, old in selected:
            ...
            print(f"signature: {signature}", file=stdout)
