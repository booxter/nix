...
toplevels and embedded Home Manager users. By default this covers:
  ...
  CA bundles, SSH moduli, profile/manpage trees, and release metadata files are
  skipped because those changes are already covered by the dix output.
...
filter_dix_output() {
  ...
  while IFS= read -r line; do
    plain_line="${line}"
    while [[ "${plain_line}" =~ ${ansi_color_re} ]]; do
...
should_skip_generated_source() {
  ...
  case "${source}" in
    */etc/profiles | */etc/profiles/* | */share/man | */share/man/* | \
      */etc/pki/tls/certs/ca-bundle.crt | */etc/ssh/moduli | \
