...
from .lidarr_repair import RepairTask
...
class RepairFileState(BaseModel):
...
class RepairJob(BaseModel):
    ...
    files: list[RepairFileState] = Field(default_factory=list)
    ...
    def load(self) -> None:
        try:
            ...
            raise PostProcessorError(
                f"cannot load Lidarr repair state {self.path}: {error}"
            ) from error
