...
def locate_source(path: Path, roots: tuple[SourceRoot, ...]) -> LocatedSource:
    try:
        ...
        raise NeedsAttention(f"Radarr source is unavailable: {error}") from error
    if not resolved.is_file() and not resolved.is_dir():
        raise NeedsAttention(f"Radarr source is not a regular file or directory: {resolved}")
    for root in roots:
    ...
    raise NeedsAttention(f"Radarr source is outside the configured roots: {resolved}")
        ...
        if path.is_file():
            ...
            for candidate in sorted(path.rglob("*")):
                if candidate.is_symlink():
                    raise NeedsAttention(f"Radarr source contains a symlink: {candidate}")
                if not candidate.is_file():
                    continue
                resolved = candidate.resolve(strict=True)
                if not resolved.is_relative_to(source.root.host_path.resolve()):
                    raise NeedsAttention(f"Radarr source escapes its configured root: {candidate}")
                stat = resolved.stat()
                entries.append(f"{resolved.relative_to(path)}\0{stat.st_size}\0{stat.st_mtime_ns}")
    except OSError as error:
        raise NeedsAttention(f"cannot fingerprint Radarr source: {error}") from error
