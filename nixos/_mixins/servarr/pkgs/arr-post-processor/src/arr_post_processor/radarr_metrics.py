...
class RadarrCollector:
    ...
    def collect(self) -> Iterable[Metric]:
        ...
        totals = Metric(
            ...
            "Radarr repair jobs handled by result.",
            "counter",
        )
...
def render_radarr_metrics(state: RepairState, *, ok: bool, now: float) -> str:
    registry = CollectorRegistry()
    registry.register(RadarrCollector(state, ok=ok, now=now))
