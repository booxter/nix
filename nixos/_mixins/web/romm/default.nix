...
let
  ...
  mediaPaths = builtins.mapAttrs (_: path: "${mediaDir}/${path}") mediaExport.layout.romm;
  ...
  rommSetupConfig = (pkgs.formats.json { }).generate "romm-setup.json" {
    image = rommImage;
    environment = commonEnvironment;
    mounts = map (mount: {
      source = mount.source;
  ...
  rommSetupCommand = utils.escapeSystemdExecArgs [
    (lib.getExe' cfg.tools.package "romm-run-setup")
    "--socket-url"
    "http+unix:///run/user/${toString rommAccount.uid}/podman/podman.sock"
    "--config"
    rommSetupConfig
...
in
{
  options = {
    ...
    services.romm = {
      enable = lib.mkEnableOption "RomM";

      dataDir = lib.mkOption {
        type = lib.types.str;
        default = "/var/lib/romm";
        description = "Directory containing RomM host-local state.";
      };

      databaseDir = lib.mkOption {
        type = lib.types.str;
        default = "/var/lib/mysql";
        description = "Directory containing RomM's MariaDB database.";
      };

      mediaDir = lib.mkOption {
        type = lib.types.str;
        default = "/var/lib/romm-media";
        description = "Local mount point for RomM media storage.";
      };

      port = lib.mkOption {
        type = lib.types.port;
        default = 5081;
        description = "Loopback port on which the RomM API listens.";
      };
      ...
      localUrl = lib.mkOption {
        ...
        readOnly = true;
        internal = true;
        description = "Loopback URL for the RomM API.";
      };

      tools.package = lib.mkOption {
        type = lib.types.package;
        default = pkgs.callPackage ./packages/romm-tools { };
        description = "Package providing RomM setup helpers.";
      };
    };
  };
  ...
  config = lib.mkMerge [
    ...
    (lib.mkIf hostCfg.enable {
      ...
      systemd.services.romm-db-init = {
        ...
        ++ tmpfilesSetupUnits;
        unitConfig.RequiresMountsFor = [ mysqlDataDir ];
        serviceConfig = {
          Type = "oneshot";
      ...
      host.backups.artifacts.mariadb.romm = {
        job = backupJob;
        displayName = "RomM";
        destinationDir = "${stateDir}-mariadb-backup/latest";
        requiresMountsFor = [
          mysqlDataDir
          stateDir
        ];
      ...
      assertions = [
        {
          assertion = instance ? dataDir && instance ? databaseDir && instance ? mediaDir;
          message = "The RomM inventory instance must define dataDir, databaseDir, and mediaDir.";
        }
        {
