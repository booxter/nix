...
let
  ...
  serviceName = "media-repair-planner";
...
in
{
  config = lib.mkIf (planner != null && planner.enable) {
    ...
    systemd.sockets.${serviceName} = {
      description = "Media repair planner API socket";
    ...
    systemd.services.${serviceName} = {
      description = "Plan media import repairs";
      wants = [
      ...
      serviceConfig = {
        ...
        ExecStart = lib.escapeShellArgs [
          (lib.getExe' planner.package "media-repair-planner-serve")
          "--backend"
          "openrouter"
          "--model"
          planner.model
