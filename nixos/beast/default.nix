...
let
  pkgsRelease = import inputs.nixpkgs-25_11 {
    system = platform;
    overlays = [
...
in
{
  ...
  nix.registry.nixpkgs.flake = inputs.nixpkgs-25_11;

  # Disable attic hook on this host for now.
  ...
  boot.kernelPackages = pkgsRelease.linuxPackages_latest;
  ...
  boot.swraid.mdadmConf = "PROGRAM ${pkgsRelease.util-linux}/bin/logger -t mdadm-monitor";
  ...
  # Samba share for media (guest access).
  services.samba = {
    enable = true;
    openFirewall = true;
    settings = {
  ...
  environment.systemPackages = with pkgsRelease; [
    btrfs-progs
    hdparm
    lm_sensors
    mdadm
