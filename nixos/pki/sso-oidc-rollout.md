...
- Native OIDC browser rollouts are complete for `ihar` in Grafana, Vikunja,
  Paperless, Open WebUI, and RomM.
- Local username/password paths remain enabled where they are needed for
  native/mobile clients, API-style access, or rollback.
...
- [x] Manually migrate existing Paperless `kasia` local account to non-staff
...
- [x] Log in as `ihar` through SSO.
- [x] Verify `ihar` receives RomM admin through `romm-admins`.
